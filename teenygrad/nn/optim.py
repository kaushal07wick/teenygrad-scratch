...
class Optimizer:
    def __init__(self, params: List[Tensor], lr: float):
        # if its None, but being put into an optimizer, set it to true
        for x in params:
    ...
    def realize(self, extra=None):
        Tensor.correalize(extra + self.params + self.buffers if extra is not None else self.params + self.buffers)
...
class LAMB(Optimizer):
    def __init__(self, params: List[Tensor], lr=0.001, b1=0.9, b2=0.999, wps=1e-6, wd=0.0, adam=False):
        super().__init__(params, lr)
