...
def train(model, X_train, Y_train, optim, steps, BS=128, lossfn=lambda out, y: out.sparse_categorical_crossentropy(y), transform=lambda x: x, target_transform=lambda x: x, noloss=False):
...
def evaluate(model, X_test, Y_test, num_classes=None, BS=128, return_predict=False, transform=lambda x: x, target_transform=lambda y: y):
...
def fetch_mnist():
    ...
    X_test = parse(BASE+"/mnists/t10k-images-idx3-ubyte.gx")[0x10:].reshape((-1, 28*28)).astype(np.float32)
...
# creating a model with a convultion layer
class TinyConvNet:
    def __init__(self):
      # https://keras.io/examples/vision/mnist_convnet/
      conv = 3
      #inner_channel, out_channel = 32, 64
      inner_chan, out_chan = 8, 16  # for speed
      self.c1 = Tensor.scaled_uniform(inner_chan, 1, conv, conv)
      self.c2 = Tensor.scaled_uniform(out_chan, inner_chan, conv, conv)
